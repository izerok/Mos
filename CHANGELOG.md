# 4.3.2 (fork: izerok/Mos, 2026-05-17)

## 修复
- 把 4.3.0/4.3.1 引入的"Scope 表格列"挪到 tableFoot 的底部工具栏: 偏好设置 → 按键 页面右下角现在有一个 `App 作用域…` 按钮 (选中 binding 后启用, 标题显示 `✓ 3` 白名单 / `✗ 0` 黑名单 + app 数, 点击弹该 binding 的 scope 配置 popover)。原方案把 scope 加成第二张表格列, 但 storyboard 窗口宽 450pt + 第一列固定 418pt 让第二列被裁到滚动视图右侧不可见。
